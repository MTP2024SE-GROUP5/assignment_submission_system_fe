...
import { useGetUrgentAssignments } from './useGetUrgentAssignments';
...
describe('useGetUrgentAssignments', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it('mounts without crashing and triggers', async () => {
        const { result } = renderHookWithProviders(() => {
            try {
                return useGetUrgentAssignments('mockId' as any, { enabled: true } as any);
            } catch {
                try {
                    return useGetUrgentAssignments();
                } catch {
                    return null;
                }
            }
