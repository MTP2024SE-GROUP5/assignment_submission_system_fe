...
import { useGetCourseDetails } from './useGetCourseDetails';
...
describe('useGetCourseDetails', () => {
    ...
    it('mounts without crashing and triggers', async () => {
        const { result } = renderHookWithProviders(() => {
            try {
                return useGetCourseDetails('mockId' as any, { enabled: true } as any);
            } catch {
                try {
                    return useGetCourseDetails();
                } catch {
                    return null;
                }
            }
