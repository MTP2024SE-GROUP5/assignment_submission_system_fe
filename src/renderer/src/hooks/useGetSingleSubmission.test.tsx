...
import { useGetSingleSubmission } from './useGetSingleSubmission';
...
describe('useGetSingleSubmission', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it('mounts without crashing and triggers', async () => {
        const { result } = renderHookWithProviders(() => {
            try {
                return useGetSingleSubmission('mockId' as any, { enabled: true } as any);
            } catch {
                try {
                    return useGetSingleSubmission();
                } catch {
                    return null;
                }
            }
