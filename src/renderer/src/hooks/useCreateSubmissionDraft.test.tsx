...
import { useCreateSubmissionDraft } from './useCreateSubmissionDraft';
...
describe('useCreateSubmissionDraft', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it('mounts without crashing and triggers', async () => {
        const { result } = renderHookWithProviders(() => {
            try {
                return useCreateSubmissionDraft('mockId' as any, { enabled: true } as any);
            } catch {
                try {
                    return useCreateSubmissionDraft();
                } catch {
                    return null;
                }
            }
