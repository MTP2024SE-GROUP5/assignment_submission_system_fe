...
import { useGetAllEnrolled } from './useGetAllEnrolled';
...
describe('useGetAllEnrolled', () => {
    ...
    it('mounts without crashing and triggers', async () => {
        const { result } = renderHookWithProviders(() => {
            try {
                return useGetAllEnrolled('mockId' as any, { enabled: true } as any);
            } catch {
                try {
                    return useGetAllEnrolled();
                } catch {
                    return null;
                }
            }
